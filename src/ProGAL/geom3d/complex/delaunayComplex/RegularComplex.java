 ...
 */
public class RegularComplex implements SimplicialComplex{
	...
	public RegularComplex(List<PointWeighted> points) {
		...
		this.predicates = new InexactRegularJavaPredicates();
	...
	protected RegularComplex(List<Point> points, Predicates predicates){
		this.points = new ArrayList<CVertex>(points.size());
		for(Point p: points) this.points.add(new CVertex(p));
		this.edges = new ArrayList<CEdge>(points.size()*6);
		this.triangles = new ArrayList<CTriangle>(points.size()*6);
