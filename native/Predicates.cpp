#include "ProGAL_geom3d_delaunay_Primitives.h"
#include <iostream>
...
/*
// inspherep3d returns positve if q is inside sphere spanned by p1,..,p4
// inspherep3d returns negative if q is outside sphere spanned by p1,..,p4
// inspherep3d returns 0 if q is on sphere spanned by p1,..,p4
REAL inspherep3d(point3d *p1, point3d *p2, point3d *p3, point3d *p4, point3d *q){
	REAL * r1 = p1->xyz;
	REAL * r2 = p2->xyz;
	REAL * r3 = p3->xyz;
	REAL * r4 = p4->xyz;
    ...
    REAL tmpres=orient3d(r1,r2,r3,r4);

	if(tmpres>0) return(insphere(r1,r2,r3,r4,rq));
	else if(tmpres<0){

		REAL res = insphere(r1,r2,r3,r4,rq);
...
// inspheretp3d returns positve if q is inside sphere spanned by tetrahedron T=p1,..,p4
// inspheretp3d returns negative if q is outside sphere spanned by tetrahedron T=p1,..,p4
// inspheretp3d returns 0 if q is on sphere spanned by tetrahedron T=p1,..,p4
REAL inspheretp3d(tetrahedron * t, point3d *q){
	point3d * p1 = t->points[0];
	point3d * p2 = t->points[1];
	point3d * p3 = t->points[2];
	point3d * p4 = t->points[3];
	return (inspherep3d(p1,p2,p3,p4,q));
}

boolean inplane(point3d * q0, point3d * q1, point3d * q2, point3d * p){
    if(orient3d(q0->xyz, q1->xyz,q2->xyz,p->xyz)==0)
        return TRUE;
    else
        return FALSE;
}

//diffsides returns true if p1 and p2 are NOT on the same side of the plane expanded by q0,q1, and q2.
boolean diffsides(point3d * q0, point3d * q1, point3d * q2,point3d * p1, point3d * p2){
	REAL a,b;
	a = orient3d(q0->xyz, q1->xyz,q2->xyz,p1->xyz);
	b = orient3d(q0->xyz, q1->xyz,q2->xyz,p2->xyz);
	return((a>0 && b<0) || (a<0 && b>0));
}


//diffsides2 returns "diff" if p1 and p2 ARE NOT on the same side of the plane expanded by q0,q1, and q2.
//diffsides2 returns "same" if p1 and p2 ARE on the same side  of the plane expanded by q0,q1, and q2.
//diffsides2 returns "coplanar" if p1 and p2 ARE IN the plane expanded by q0,q1, and q2.
side diffsides2(point3d * q0, point3d * q1, point3d * q2,point3d * p1, point3d * p2){
    REAL a,b;
	a = orient3d(q0->xyz, q1->xyz,q2->xyz,p1->xyz);
	b = orient3d(q0->xyz, q1->xyz,q2->xyz,p2->xyz);

	if((a>0 && b<0) || (a<0 && b>0)) return DIFF;
	if((a>0 && b>0) || (a<0 && b<0)) return SAME;
	if(a==0 || b==0) return COPL;
}

// insidet returns true if q is on the same side of the four planes spanned by a tetrahedron t
// otherwise false
boolean insidet(tetrahedron * t, point3d *q){
	point3d * p1 = t->points[0];
	point3d * p2 = t->points[1];
	point3d * p3 = t->points[2];
	point3d * p4 = t->points[3];

	return( !diffsides(p1,p2,p3,q,p4) && !diffsides(p2,p3,p4,q,p1) && !diffsides(p1,p2,p4,q,p3) && !diffsides(p1,p3,p4,q,p2) );
}
*/
/*
 * Class:     Predicates_Primitives
 * Method:    exactinit
 * Signature: ()V
 */
JNIEXPORT void JNICALL Java_ProGAL_geom3d_delaunay_Primitives_exactinit(JNIEnv *, jobject){
	exactinit();
}
 ...
 */
JNIEXPORT jdouble JNICALL Java_ProGAL_geom3d_delaunay_Primitives_insphere(JNIEnv *, jobject, jdouble x1, jdouble y1, jdouble z1, jdouble x2, jdouble y2, jdouble z2, jdouble x3, jdouble y3, jdouble z3, jdouble x4, jdouble y4, jdouble z4, jdouble x5, jdouble y5, jdouble z5){
 ...
 */
JNIEXPORT jdouble JNICALL JNICALL Java_ProGAL_geom3d_delaunay_Primitives_orient3d(JNIEnv *, jobject, jdouble x1, jdouble y1, jdouble z1, jdouble x2, jdouble y2, jdouble z2, jdouble x3, jdouble y3, jdouble z3, jdouble x4, jdouble y4, jdouble z4){
p0[0]=x1;
p0[1]=y1;
p0[2]=z1;
...
/*  tetcircumradius()   Find the radius of circumsphere of a tetrahedron.    */
/*                                                                           */
...
JNIEXPORT jdouble JNICALL Java_ProGAL_geom3d_delaunay_Primitives_tetcircumradius(JNIEnv *, jobject, jdouble a1, jdouble a2, jdouble a3, jdouble b1, jdouble b2, jdouble b3, jdouble c1, jdouble c2, jdouble c3, jdouble d1, jdouble d2, jdouble d3){
 double a[3]; double b[3]; double c[3]; double d[3]; double circumcenter[3];
    double t1,t2,t3;
...
JNIEXPORT jdouble JNICALL Java_ProGAL_geom3d_delaunay_Primitives_tricircumradius3d(JNIEnv *, jobject, jdouble a1, jdouble a2, jdouble a3, jdouble b1, jdouble b2, jdouble b3, jdouble c1, jdouble c2, jdouble c3){
    double a[3]; double b[3]; double c[3]; double circumcenter[3];
    double t1,t2,t3;
    a[0] = a1; a[1] = a2; a[2] = a3;
    b[0] = b1; b[1] = b2; b[2] = b3;
    c[0] = c1; c[1] = c2; c[2] = c3;
...
/*
Checks whether a point q is inside (returns greater then zero), surface (returns 0) or outside (returns less then zero)
*/
JNIEXPORT jdouble JNICALL Java_ProGAL_geom3d_delaunay_Primitives_inspheretri  (JNIEnv *, jobject, jdouble a1, jdouble a2, jdouble a3, jdouble b1, jdouble b2, jdouble b3, jdouble c1, jdouble c2, jdouble c3, jdouble q1, jdouble q2, jdouble q3){
    double a[3]; double b[3]; double c[3]; double q[3]; double circumcenter[3];
    double t1,t2,t3,r;
    double s1,s2,s3,u;
    a[0] = a1; a[1] = a2; a[2] = a3;
    b[0] = b1; b[1] = b2; b[2] = b3;
    ...
    //compute distance² from circumcenter to q
